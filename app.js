...
app.post("/newtopic", (request, response) => {
  ...
  db.collection(topicCollectionName).findOne({ title: topic.title }, function(
    err,
    results
  ) {
    if (err) {
...
app.post("/newuser", (request, response) => {
  ...
  if (user.topics) {
    for (index in user.topics) {
      ...
      db.collection(topicCollectionName).findOne(
        { title: topictitle },
...
app.post("/newconversation", (request, response) => {
  ...
  db.collection(userCollectionName).findOne({ _id: conversation.id1 }, function(
    err,
    result
  ) {
    if (err) {
  ...
  db.collection(userCollectionName).findOne({ _id: conversation.id2 }, function(
    err,
    result
  ) {
    if (err) {
  ...
  db.collection(conversationCollectionName).insertOne(
    insertConversation,
    function(err, result) {
      if (err) {
...
app.get("/inbox", (request, response) => {
  ...
  db.collection(userCollectionName).findOne({ _id: id }, function(err, result) {
    ...
    db.collection(conversationCollectionName)
      .find(query, options)
      .sort({ updated: -1 })
      .toArray(function(err, results) {
        if (err) {
...
app.get("/conversation", (request, response) => {
  ...
  if (cid) {
    db.collection(conversationCollectionName).findOne({ _id: cid }, function(
      err,
      result
    ) {
      if (err || result == null) {
      ...
      var options = {};
      db.collection(messageCollectionName)
        .find(query, options)
        .sort({ created: -1 })
        .toArray(function(err, results) {
          if (err) {
    ...
    db.collection(conversationCollectionName).findOne(query, function(
      err,
      result
    ) {
      if (err || result == null) {
...
app.post("/newmessage", (request, response) => {
  ...
  db.collection(userCollectionName).findOne({ _id: senderid }, function(
    err,
    result
  ) {
    if (err || result == null) {
    ...
    db.collection(conversationCollectionName).findOne({ _id: cid }, function(
      ...
      db.collection(messageCollectionName).insert(insertMessage, function(
        err,
        result
      ) {
        if (err) {
